...
export function isNumeric(value: any): boolean {
	return testConstructor('Number', value) && !isNaN(value);
}
...
export function isGUIDv4(value: any): boolean {
	return isString(value)
	       && /^(?:\{[\dA-F]{8}-[\dA-F]{4}-4[\dA-F]{3}-[89AB][\dA-F]{3}-[\dA-F]{12}}|[\dA-F]{8}-[\dA-F]{4}-4[\dA-F]{3}-[89AB][\dA-F]{3}-[\dA-F]{12})$/i.test(value);
}

/** Is value a valid GUID *string* */
export function isGUID(value: any): boolean {
	return isString(value)
	       && /^(?:\{[\dA-F]{8}-[\dA-F]{4}-[\dA-F]{4}-[\dA-F]{4}-[\dA-F]{12}}|[\dA-F]{8}-[\dA-F]{4}-[\dA-F]{4}-[\dA-F]{4}-[\dA-F]{12})$/i.test(value);
}
